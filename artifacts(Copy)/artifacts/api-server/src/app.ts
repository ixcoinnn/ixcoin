...
if (isProdMode) {
  const distDir = process.env["STATIC_DIR"] ?? path.resolve(__dirname, "../../public");
  if (existsSync(distDir)) {
    app.use(express.static(distDir, { maxAge: "1y", etag: true }));
    app.get("*", (_req, res) => {
      res.sendFile(path.join(distDir, "index.html"));
    });
    logger.info({ distDir }, "Serving static frontend from dist");
  } else {
    logger.warn({ distDir }, "Static frontend dist not found — serving API only");
